...
;(function(root, factory) {
    ...
    var jet = {}, doc = document, regymdzz = "YYYY|MM|DD|hh|mm|zz",
        regymd = "YYYY|MM|DD|hh|mm|zz".replace("|zz",""),
    ...
    jedfn.init = function(){
        ...
        var initVals = function(objCell) {
            ...
            if (jet.isObj(hmsset)){
                jeaddDate = jet.parse([ newDate.getFullYear(), jet.digit(newDate.getMonth() + 1), jet.digit(newDate.getDate()) ], [ jet.digit(hmsset.hh), jet.digit(hmsset.mm) ], jeformat);
            }else{
    ...
    jedfn.generateHtml = function(opts,boxCell){
        ...
        var hmsStrHtml = '</div>'+'</div>' +'<div class="jedatehmscon jedateprophours"></div></div>';
        var botStrHtml = '<div class="tiin">时间</div><div class="botflex jedatehmsshde"><ul class="jedatehms"><li><input type="text" ></li></ul></div>' +
            '<div class="botflex jedatebtn"><span class="jedateok">'+lang.yes+'</span></div>';
        ...
        if(/\hh/.test(jet.format)) {
            ...
            var hmsarr = that.isContainhh(jet.format),hmsset = opts.hmsSetVal||{},
                hmsobj = ((objCell.val() || objCell.text()) == "" && jet.isObj(hmsset)) ? hmsset : {"hh":tmsArr[3], "mm":tmsArr[4]};
            $.each(["hh", "mm"], function (i, hms) {
    ...
    jedfn.eachStrhms = function(opts,boxCell) {
        ...
        $.each([ 24, 60, 60 ], function(i, len) {
            ...
            for (var h = 0; h < len; h++) {
                ...
                hmsStr += '<p class="' + hmsCls + '">' + h + ':' + '00' + "</p>";
                hmsStr += '<p class="' + hmsCls + '">' + h + ':' + '30' + "</p>";
            }
            hmsArr.push(hmsStr);
        });
        return hmsArr;
    ...
    jedfn.orien = function(obj, self, pos) {
        ...
        if(jet.fixed) {
            ...
            ortop = Math.max(tops + (pos ? 0 :jet.docScroll()) + 1, 1) - 0 + "px", orleri = leris + "px";
        }else{
            //弹层位置位于页面上下左右居中
            ortop = "50%", orleri = "50%";
            ...
            var sethmsStrHtml = function () {
                ...
                $.each([ "hours", "minutes", "seconds" ], function(i, hms) {
                    ...
                    onhmsPCls.on("click", function() {
                        ...
                        boxCell.find(".jedatebot .jedatehms input").val(jet.digit(_this.text()));
        ...
        boxCell.find(".jedatebot .jedateok").on("click", function(ev) {
            ...
            var date = new Date(),
                okhms = (function() {
                    ...
                    // });
                    var disattr = $(this).attr('disabled');
                    hmsArr=$(".jedatehms input").val().split(':');
                    return hmsArr;
                })();
            var okymd = ishhmat ? [date.getFullYear(),date.getMonth() + 1,date.getDate()] : jet.reMacth(boxCell.children("ul").attr("dateval")),
                ...
                okVal = jet.parse([parseInt(okymd[0]), parseInt(okymd[1]), parseInt(okymd[2])], [okhms[0]||00,okhms[1]||00], okformat),
                okdate = (okymd[0]||date.getFullYear())+"-"+jet.digit(okymd[1]||date.getMonth() + 1)+"-"+jet.digit(okymd[2]||date.getDate())+" "+jet.digit(okhms||00);
    ...
    jedfn.chooseDays = function(opts,boxCell) {
        ...
        boxCell.find(".jedaul li").on("click", function(ev) {
            ...
            date = new Date();
            if(liTms==""){
                liTms=[date.getHours(),date.getMinutes()]
            }
            else{
                 liTms=$(".jedatehms input").val();
            }
            //获取时分秒的集合
            var dateArr = jet.reMacth(_that.attr("data-ymd")),

                getDateVal = jet.parse([ dateArr[0], dateArr[1], dateArr[2] ], [ liTms[0], liTms[1] ], thisformat),
                wdate = (dateArr[0]||date.getFullYear())+"-"+jet.digit(dateArr[1]||date.getMonth() + 1)+"-"+jet.digit(dateArr[2]||date.getDate())+" "+jet.digit(liTms[0])+" "+jet.digit(liTms[1]);
    ...
    $.timeStampDate = function (date,bool,format) {
        ...
        if(bool == true){  //将时间戳转换成日期
            ...
            return jet.parse([ setdate.getFullYear(), jet.digit(setdate.getMonth()), jet.digit(setdate.getDate()) ], [ jet.digit(setdate.getHours()), jet.digit(setdate.getMinutes())], format);
        }else {  //将日期转换成时间戳
